...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        **kwargs
        ...
        iris_faces_idx = np.loadtxt(os.path.join(model_path, 'iris_faces_ids.txt'), dtype=np.int64)
        iris_bary_coords = np.loadtxt(os.path.join(model_path, 'iris_bary_coords.txt'), dtype=np.float32)

        self.register_buffer('iris_faces_idx',
                             torch.tensor(iris_faces_idx, dtype=torch.long))
        self.register_buffer('iris_bary_coords',
                             torch.tensor(iris_bary_coords, dtype=dtype))
    ...
    def forward(
        ...
        **kwargs
        ...
        iris_faces_idx = self.iris_faces_idx.unsqueeze(
            dim=0).expand(batch_size, -1).contiguous()
        iris_bary_coords = self.iris_bary_coords.unsqueeze(dim=0).repeat(
            self.batch_size, 1, 1)
        iris_landmarks = vertices2landmarks(vertices, self.faces_tensor,
                                       iris_faces_idx,
                                       iris_bary_coords)
        ...
        if (not with_rott_return) and (not with_iris_return): 
            return output
        if with_iris_return:
            return output, iris_landmarks
        if with_rott_return:
